(define (script-fu-spiral-square inImage inLayer inStepPerc inSideLenPass inOverSample)
...
(let* 
	...
	(let* 
		...
		(while (> localSideLen  10)	
			(script-fu-draw-square2 theLayer localSideLen (* localSideLen 0.01) (/ inSideLen 2) (/ inSideLen 2) radRot inLayer)
...
(define (script-fu-paste-rivet inLayer outLayer xMin yMin xMax yMax)
	(let* 
	    (
	    	(test 0)
			(selection)
...
(define (script-fu-draw-square2 theLayer sideLen penWidth centreX centreY radRot inLayer)
	(let*
		; Local variables 
		(
		...
		(gimp-brush-set-hardness theBrushName 1)
		...
		(let*;
			...
			(while (< sideNum 4)
			 	...
			 	;normalise
			 	(set! normalX (/ normalX normalMag))
			 	(set! normalY (/ normalY normalMag))
			 	(set! normalZ (/ normalZ normalMag))

			 	;Convert to RGB style value
			 	(set! normalX (+ 127  (* normalX 127) ) )
			 	(set! normalY (+ 127  (* normalY 127) ) )
			 	(set! normalZ (+ 127  (* normalZ 127) ) )
			 	
			 	(set! color (list normalX normalY normalZ))
				...
				;(gimp-message (string-append "X = " (number->string normalX) "Y = " (number->string normalY) ))
			 	...
			 	(let* (
			 		...
			 		(script-fu-paste-rivet inLayer theLayer xMin yMin xMax yMax)
			 	)	
...
(script-fu-register
	"script-fu-spiral-square"						; name of the function
	"Spiral Square"									; Menu label
...
(script-fu-menu-register "script-fu-spiral-square" "<Image>/File/Create/Text")