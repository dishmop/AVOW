...
public class SetupResolution : MonoBehaviour {
	...
	void ReduceResolution(){
		...
		if (newResIndex != resIndex){
			...
			GoogleAnalytics.Client.SendTimedEventHit("reduceResolution",  AVOWGameModes.singleton.GetCurrentLevelName(), (resolutions[newResIndex].width + "x" + resolutions[newResIndex].height), Time.fixedTime);
//						
