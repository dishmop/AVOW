...
using UnityEditor;
using UnityEngine;
[CanEditMultipleObjects, CustomEditor(typeof(Gear))]
...
public class GearInspector : Editor {
	...
	static void CreateNewGear() {
		...
		go.name = "Gear";
		go.AddComponent(typeof(Gear));
		Selection.activeGameObject = go;
		go.transform.position = position;

// vvvv  Uncomment this line if you want to use with the TransMission Gear Tool  vvvv
		
//		go.AddComponent(typeof(TransMission));
...
// vvvv  Uncomment this lines if you want to use with the TransMission Gear Tool  vvvv 
//		
//		SerializedProperty
//			dk			= prefs.FindPropertyRelative("dk"),
//		    d			= prefs.FindPropertyRelative("d");
//		foreach(Gear g in targets){
//			TransMission tm = (TransMission) g.gameObject.GetComponent(typeof(TransMission));
//				tm.dk = dk.floatValue*2.0f;
//				tm.diameter = d.floatValue;
//				tm.teethCount = teethCount.intValue;
//		}
//		
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	private void ShowInfoField() {
		...
		if (v != 0)
			 InfoField ("  Vertices: " + v.ToString() + "    " + str +
					  ...
					  "\n  CPU time: " +  str2 + t.ToString() + "ms" + str3, new Color(0.85f,0.85f,0.85f),220f);
